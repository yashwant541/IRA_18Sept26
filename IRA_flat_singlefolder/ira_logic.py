...
def _inherent_shortfall_sheet(wb, frames, tables):
    ...
    def band(rr, cc, txt, style="BODY"):
        cell = ws.cell(rr, cc, txt); cell.font = S[style]; cell.alignment = S["WRAP"]
        cell.border = S["BORDER"]

    # ---- Calculated Inherent, per product per country ----
    try:
        inh = G.inherent_trace(frames)
    except Exception as e:
        inh = {}
        band(r, 1, "inherent trace unavailable: " + str(e)); r += 1
    for product in PRODUCTS:
        rows = inh.get(product)
        if not rows:
            continue
        for cc in range(1, 6):
            ws.cell(r, cc).fill = S["FILL"]; ws.cell(r, cc).border = S["BORDER"]
        band(r, 1, "INHERENT - " + product, "SUB"); r += 1
        for e in rows:
            band(r, 1, e["country"], "BOLD")
            band(r, 2, "Calculated Inherent Credit Risk Assessment", "BODY")
            band(r, 3, e["score"], "BOLD")
            rt = str(e["rating"] or "")
            rc = ws.cell(r, 4, rt); rc.font = S["BOLD"]; rc.border = S["BORDER"]
            rc.fill = PatternFill("solid", fgColor=RATING_FILL.get(rt, "FFFFFF"))
            band(r, 5, "")
            r += 1
    ...
    # ---- Shortfall arithmetic, per country ----
    try:
        sf = G.shortfall_trace(frames, tables)
    except Exception as e:
        sf = []
        band(r, 1, "shortfall trace unavailable: " + str(e)); r += 1
    if sf:
        for cc in range(1, 6):
            ws.cell(r, cc).fill = S["FILL"]; ws.cell(r, cc).border = S["BORDER"]
        band(r, 1, "SHORTFALL (Wealth Lending & PvB)", "SUB"); r += 1
        for e in sf:
            band(r, 1, e["country"], "BOLD")
            band(r, 2, (f"securities {e['securities']} + real-estate {e['real_estate']} = "
                        f"{e['sum_usd000']}  /1000 = {round((e['sum_usd000'] or 0)/1000.0,4)}  "
                        f"/  ENR PvB {round(e['enr_pvb'],2) if e['enr_pvb'] else e['enr_pvb']}"))
            band(r, 3, (f"{e['ratio']*100:.2f}%" if e["ratio"] is not None else ""), "BOLD")
            r += 1
    ws.freeze_panes = "A5"
...
def build_trace_workbook(frames, intermediates, tables=None) -> bytes:
    ...
    _inherent_shortfall_sheet(wb, frames, tables)
