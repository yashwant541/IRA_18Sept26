...
# denominators.  Wealth Lending & PvB use Wealth Banking + PvB; Retail Banking
# uses Wealth Banking only (Wealth Lending exposure, excluding Private Banking).
CATEGORY_ENR_LINES = {
    "Secured": ["Consumer Secured"],
    "Unsecured": ["Consumer Unsecured"],
    "SME Banking": ["SME Banking"],
    "Wealth Lending": ["Wealth Banking", "PvB"],
    "Wealth Lending - Retail Banking": ["Wealth Banking"],
    ...
    for name, frame in frames.items():
        ...
        for ctry in list(dict.fromkeys(frame[ctry_col].tolist())):
            ...
            for positions in groups:
                labs = [ordered[p - 1] for p in positions if 1 <= p <= len(ordered)]
                pairs = [(lb, rn.get(lb)) for lb in labs if lb not in INHERENT_EXCLUDE_CANON]
                valid = [n for _, n in pairs if n is not None]
                mx = max(valid) if valid else None
                if mx is not None:
                    score += weight * mx
                detail.append((labs, mx, (round(weight * mx, 4) if mx is not None else 0)))
            calc = frame[(frame[ctry_col] == ctry) &
                         (frame[lab_col].astype(str).str.startswith("Calculated"))]
            stored = calc.iloc[0]["Risk Rating"] if len(calc) else ""
            rows.append({"country": ctry, "weight": weight, "themes": detail,
                         "score": round(score, 4), "rating": stored or _score_to_rating(score if score else None)})
        out[product] = rows
    return out


def shortfall_trace(frames, tables):
    """Per country (Wealth Lending scope): the shortfall proportion arithmetic -
    securities + real-estate Total-row amounts, /1000, over ENR PvB."""
    wm = tables.get("wm_shortfall") or {}
    if not wm:
        return []
    enr = tables.get("ENR")
    month = _at(_months(enr), 0)
    frame = frames.get("IRA - Wealth Lending")
    rows = []
    countries = []
    if frame is not None and not frame.empty:
        countries = [c for c in dict.fromkeys(frame["Country"].tolist()) if c != GROUP_COUNTRY]

    def amt(tbl, country):
        d = wm.get(tbl) or {}
        if country in d:
            return _num(d[country])
        ck = getattr(E, "country_key", lambda x: str(x).lower())(country)
        for k, v in d.items():
            if k != "__total__" and getattr(E, "country_key", lambda x: str(x).lower())(k) == ck:
                return _num(v)
        return None

    for ctry in countries:
        sec, re_ = amt("securities", ctry), amt("real_estate", ctry)
        if sec is None and re_ is None:
            continue
        total = (sec or 0) + (re_ or 0)
        den = None
        if enr is not None:
            s = enr.series_pp(ctry, "PvB")
            den = s.get(month) if s else None
        ratio = ((total / 1000.0) / den) if den else None
        rows.append({"country": ctry, "securities": sec, "real_estate": re_,
                     "sum_usd000": total, "enr_pvb": den, "ratio": ratio})
    # GROUP total row
    sec = _num((wm.get("securities") or {}).get("__total__"))
    re_ = _num((wm.get("real_estate") or {}).get("__total__"))
    if sec is not None or re_ is not None:
        total = (sec or 0) + (re_ or 0)
        den = _sum_product_at(enr, "PvB", month)
        ratio = ((total / 1000.0) / den) if den else None
        rows.append({"country": "GROUP", "securities": sec, "real_estate": re_,
                     "sum_usd000": total, "enr_pvb": den, "ratio": ratio})
    return rows
...
def _ratio_detail(tables, product, int_key, val):
    ...
    if int_key == "shortfall":
        ...
        return [("securities Total-Amount", sec), ("real-estate Total-Amount", re_),
                ...
                ("sum ENR Private Banking/PvB (all countries)", round(den, 2) if den else den),
                ("= ratio", _fmt_pct(val))]
